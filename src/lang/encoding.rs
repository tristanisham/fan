...
/// base64_encode encodes bytes as Base64.
pub extern "C" fn base64_encode(vm: *mut WrenVM) {
    unsafe {
        wren::wrenEnsureSlots(vm, 2);
    }

    let input = unsafe { wren::wrenGetSlotString(vm, 1) };
    ...
    let ce_ptr = c_encoded.as_ptr();
    unsafe {
        wren::wrenSetSlotString(vm, 0, ce_ptr);
    }
}

// Function to free the memory allocated by base64_encode
/// base64_decode decodes bytes from Base64.
pub extern "C" fn base64_decode(vm: *mut WrenVM) {
    unsafe {
        wren::wrenEnsureSlots(vm, 2);
    }
    let input = unsafe { wren::wrenGetSlotString(vm, 1) };
