...
impl From<u32> for VoidSlotType {
...
impl Runtime {
    pub fn execute(mut self, code: &str) -> Result<(), VoidInterpretError> {
        unsafe {
            ...
            if result == 1 {
                return Err(VoidInterpretError::CompileError);
            } else if result == 2 {
                return Err(VoidInterpretError::RuntimeError);
            }
